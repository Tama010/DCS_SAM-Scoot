...
function avoidanceBomb()
    ...
    local _DETECTRANGE_S = 8000
    local _DETECTRANGE_M = 15000
    local _DETECTRANGE_L = 18000
    ...
    function canDetectBomb(_bomb, _grp)
        ...
        for _, _unit in pairs(_grp:getUnits()) do
            ...
            if _unit:getRadar() then
                ...
                -- ついでに退避可能な距離かも判別
                if _distance < judgeDetectionRange(_unit) and _distance > _GIVEUPRANGE then
                    return true
                end
            end
        end
    ...
    -- レーダー能力の判定
    function judgeDetectionRange(_unit)
        if _unit:isExist() then
            if _unit:getTypeName() == "Osa 9A33 ln" 
            or _unit:getTypeName() == "Tor 9A331" 
            or _unit:getTypeName() == "2S6 Tunguska"
            or _unit:getTypeName() == "Gepard" then
                return _DETECTRANGE_S
            elseif _unit:getTypeName() == "SA-11 Buk SR 9S18M1" then
                return _DETECTRANGE_L
            else
                return _DETECTRANGE_M
            end
        end
        return 0
    end
    ...
    function scoot(_scootFlagStockObj)
        ...
        if isMovingLaunchSam(_scootFlagStockObj) then
            if _scootFlagStockObj['grp']:isExist() then
                trigger.action.groupContinueMoving(_scootFlagStockObj['grp'])
            end
        else
            -- アラートステートをグリーン
    ...
    -- 移動しながら射撃は可能なSAMか
    function isMovingLaunchSam(_scootFlagStockObj)

        if _scootFlagStockObj['grp']:isExist() then
            for _, _unit in pairs(_scootFlagStockObj['grp']:getUnits()) do
                if _unit:isExist() then
                    if _unit:getTypeName() == "Osa 9A33 ln" 
                    or _unit:getTypeName() == "Tor 9A331" 
                    or _unit:getTypeName() == "2S6 Tunguska" 
